...
- **断点续写**：每章自动保存到本地，浏览器断开或电脑息屏后可从断点恢复，无需重新上传和分析
...
│   └── project.py          # 项目管理（自动保存/断点续写/恢复）
├── projects/               # 写作项目存档（自动创建）
├── memory/
...
### 6. 断点续写
- 分析、大纲、每章正文都会自动保存到 `projects/{书名}/` 目录
- 写作中途关闭浏览器或电脑息屏，已写章节全部保留
- 重新打开系统时自动检测未完成项目，选择恢复即可从断点继续
- 写作过程中可随时下载当前进度
